...
void parse_edge_list_with_weights(string input_file, const vector<float>& weights, 
    vector<vector<int>>& input_graph, vector<vector<float>>& input_graph_weigths) {
    ...
    if (file.is_open()) {
        ...
        input_graph.resize(vertices);
        input_graph_weigths.resize(vertices);
        ...
        while (Getline(file, line))
        {
            ...
            if (words[0] == "e") {
                ...
                input_graph_weigths[node1].push_back(weights[edgeId]);
                input_graph_weigths[node2].push_back(weights[edgeId]);
